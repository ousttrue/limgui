    ...
    if isglfw then
        ...
        Lib.glext = setmetatable({}, {
            __index = function(self, k)
                ...
                if not ok then
                    error("Couldn't find pointer type for " .. k .. " (are you accessing the right function?)", 2)
                end

                local ptr = ffi.cast(typ, glfw.glfwGetProcAddress(k))
                if ptr == nil then
                    error("Unable to load function: " .. k, 2)
                end
